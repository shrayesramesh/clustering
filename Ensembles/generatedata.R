...
iterationoutput<- sapply(1:H, function(x) {
  ...
  #s<- sample(1:dim(smiley)[1],800)
  s <- 1:dim(datamatrix)[1]
...
##schema:
##lift_cluster, lift_x, lift_y, jaccard_cluster, jaccard_x, jaccard_y

outmat <- do.call(cbind,lapply (list(hyp=hypergraph), function(g){
  #dmat <- as.matrix(1-g)
  dmat <- exp(-g)
  l4p <- cmdscale(dmat)*100
  #affinity = ifelse(as.matrix(g)>quantile(as.vector(g),1-1/H),1,0.0001)
  #affinity = exp(-dmat /(.05 * sd(dmat)^2))
  affinity = g
